  ...
  const handleFormSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    salvarCliente(clienteEdit)
      .then(respCliente => {
        ...
        //se for inclusão redireciona para edição
        if (clienteEdit.token === '') window.location.href = `/cliente/view/${respCliente.token}`
      })
      .catch(err => {
    ...
    <Card className='relative'>
      <form onSubmit={e => handleFormSubmit(e)}>
        ...
        <CardContent className='flex flex-col gap-4'>
          <Grid container spacing={5}>
            <Grid item xs={12}>
              <CustomTextField fullWidth label='CEP' value={clienteEdit?.cep} onChange={e => handleCepChange(e)} />
            </Grid>
            <Grid item xs={12} sm={6}>
              <CustomTextField
                fullWidth
